...
public class AdapterPhysican extends FirebaseRecyclerAdapter<Modalsub,AdapterPhysican.ViewHolder> {
    public AdapterPhysican(@NonNull FirebaseRecyclerOptions<Modalsub> options) {
        super(options);
    }

    @Override
    protected void onBindViewHolder(@NonNull ViewHolder holder, int i, @NonNull Modalsub modalsub) {
        if ("Consultant Physician".equals(modalsub.getProfession())){
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View physicanview= LayoutInflater.from(parent.getContext()).inflate(R.layout.profile_layout,parent,false);
        return new ViewHolder(physicanview);
    }
